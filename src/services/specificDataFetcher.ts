...
export async function fetchSpecificData(query: string): Promise<object> {
  ...
  try {
    ...
    // Extract and filter meaningful elements
    const elementsWithId = await page.$$eval('[id]', (elements: Element[]) => {
      ...
      elements.forEach((element) => {
        ...
        if (
          id && // Ensure `id` exists
          textContent && textContent !== 'No text content available' && // Exclude empty or placeholder text
          !id.includes('google-tag-manager') && // Exclude tracking scripts
          !id.includes('ads') // Exclude ads
        ) {
