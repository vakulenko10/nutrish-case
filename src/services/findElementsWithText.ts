...
export async function findElementsWithText(query: string, text: string): Promise<object> {
  ...
  try {
    ...
    // Extract elements containing the target text
    const elementsWithText = await page.$$eval('[id]', (elements: Element[], targetText: string) => {
      const result: { [key: string]: string } = {};
      elements.forEach((element) => {
        const id = (element as HTMLElement).id;
        const textContent = element.textContent?.trim();

        // Check if the text content includes the target text
        if (id && textContent && textContent.toLowerCase().includes(targetText.toLowerCase())) {
    ...
    return { query, text, elementsWithText };
  } catch (error: any) {
    console.error(`Error while finding elements with text: ${error.message}`);
